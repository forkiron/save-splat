...
const payload: Record<string, unknown> = {
  ...
  geometry: {
    planes: [
      ...
      { label: 'W2', cls: 'wall', support: 0.08, tilt_deg: 1.1, drift_ratio: 0.019, drift_band: 'MODERATE' },
...
test('citation check rejects paths that are not in the payload', () => {
  const v = verify({
    key: 'morphology',
    param: 'type',
    proposal: prop({ evidence_used: ['geometry.planes[0].cls', 'geometry.thermal_imaging.hotspots'] }),
    payload,
  });
...
test('proposing a value while citing nothing fails', () => {
  const v = verify({ key: 'morphology', param: 'type', proposal: prop({ evidence_used: [] }), payload });
  assert.equal(v.find((x) => x.check === 'citations')!.status, 'fail');
});

test('contract check catches incoherent abstain/value pairs', () => {
  const a = verify({ key: 'morphology', param: 'type', proposal: prop({ abstain: true, value: 'mixed' }), payload });
  assert.equal(a.find((x) => x.check === 'contract')!.status, 'fail');
  const b = verify({ key: 'morphology', param: 'type', proposal: prop({ abstain: false, value: null }), payload });
  assert.equal(b.find((x) => x.check === 'contract')!.status, 'fail');
});

test('morphology: lean-to with no steep inclined plane fails', () => {
  const thin = { ...payload, geometry: { ...(payload.geometry as object), planes: [
    { label: 'S1', cls: 'slab', support: 0.3, tilt_deg: 2 },
  ] } };
  const v = verify({ key: 'morphology', param: 'type', proposal: prop({ value: 'lean' }), payload: thin });
...
test('morphology: pancake fails when vertical structure dominates', () => {
  const walls = { ...payload, geometry: { ...(payload.geometry as object), planes: [
    { label: 'W1', cls: 'wall', support: 0.3, tilt_deg: 1 },
    { label: 'S1', cls: 'slab', support: 0.05, tilt_deg: 2 },
  ] } };
  const v = verify({ key: 'morphology', param: 'type', proposal: prop({ value: 'pancake' }), payload: walls });
  assert.equal(v.find((x) => x.check === 'plane-consistency')!.status, 'fail');
});

test('morphology: lean-to passes with a genuine steep incline', () => {
  const v = verify({ key: 'morphology', param: 'type', proposal: prop({ value: 'lean' }), payload });
...
test('access: high r under a severe-drift wall is contradictory', () => {
  const severe = { ...payload, geometry: { ...(payload.geometry as object), planes: [
    { label: 'W1', cls: 'wall', support: 0.1, drift_band: 'SEVERE', tilt_deg: 6 },
  ] } };
...
test('records: abstaining with no occupancy source is the correct outcome', () => {
  const v = verify({ key: 'records', param: 'n', proposal: prop({ abstain: true, value: null, evidence_used: [] }), payload });
...
test('records: a value from notes must actually cite the notes', () => {
  ...
  const uncited = verify({ key: 'records', param: 'n', proposal: prop({ value: 20 }), payload: withNotes });
...
test('corroboration must flag low when an upstream agent failed or abstained', () => {
  ...
  const high = verify({ key: 'corroboration', param: 'conf', proposal: prop({ value: 'high' }), payload, upstream: up });
  assert.equal(high.find((x) => x.check === 'agreement')!.status, 'fail');
  const low = verify({ key: 'corroboration', param: 'conf', proposal: prop({ value: 'low' }), payload, upstream: up });
...
test('corroboration may say high only when everything upstream held', () => {
  ...
  const v = verify({ key: 'corroboration', param: 'conf', proposal: prop({ value: 'high' }), payload, upstream: up });
...
test('output schema rejects out-of-range values rather than clamping them', () => {
  ...
  const bad = S.safeParse({ abstain: false, value: 500, self_confidence: 'high', rationale: 'x', evidence_used: [] });
  assert.equal(bad.success, false);
  const frac = S.safeParse({ abstain: false, value: 8.5, self_confidence: 'high', rationale: 'x', evidence_used: [] });
